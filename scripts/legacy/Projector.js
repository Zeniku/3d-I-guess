...
static chunkProjectTriangles(world) {
  ...
  for (let c = 0; c < cLength; c++) {
    ...
    this.projectedTriangles.forEach(t => {
      ...
      // Default colors if something goes wrong
      let c0 = [1, 1, 1, 1];
      let c1 = [1, 1, 1, 1];
      let c2 = [1, 1, 1, 1];

      // Grab the specific color for each vertex of this triangle
      if (chunk && chunk.pointCol) {
          c0 = chunk.pointCol[t[0][3]] || c0;
          c1 = chunk.pointCol[t[1][3]] || c1;
          c2 = chunk.pointCol[t[2][3]] || c2;
      }
      let avgZ = (t[0][2] + t[1][2] + t[2][2]) / 3;
    let fogDensity = 0.005; // Adjust this to change fog thickness
    
    // 2. Fog factor (0 is no fog, 1 is full sky color)
    let fogFactor = 1.0 - Math.exp(-avgZ * fogDensity);
    fogFactor = Math.min(Math.max(fogFactor, 0), 1);

    const skyCol = [135/255, 206/255, 235/255]; // Match your gl.clearColor

    // 3. Apply fog to each vertex color
    [c0, c1, c2].forEach(col => {
        col[0] = col[0] * (1 - fogFactor) + skyCol[0] * fogFactor;
        col[1] = col[1] * (1 - fogFactor) + skyCol[1] * fogFactor;
        col[2] = col[2] * (1 - fogFactor) + skyCol[2] * fogFactor;
    });
      ...
      if (!wireframe) {
        ...
        // Push the 3 distinct vertex colors! WebGL will interpolate them.
        Draw.batch.push(t[0][0], t[0][1], 0, 0, ...c0);
        Draw.batch.push(t[1][0], t[1][1], 0, 0, ...c1);
        Draw.batch.push(t[2][0], t[2][1], 0, 0, ...c2);
      }
    });
  }
